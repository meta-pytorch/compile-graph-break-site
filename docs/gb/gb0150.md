---
layout: default
---
# GB0150

## Graph-Break Type
*Short name describing what triggered the graph break*

Unsupported hasattr call

## Context
*Values or code snippet captured at the break point*

call_obj_hasattr {self} {name}

## Explanation
*Explanation of why the graph break was triggered*

Dynamo does not know how to trace the function `{self.debug_repr()}`

## Hints
*Hints on how to resolve the graph break*

- Avoid calling `hasattr({self.__class__.__name__}, {name})` in your code.
- It may be possible to write Dynamo tracing rules for this code. Please report an issue to PyTorch if you encounter this graph break often and it is causing performance issues.


## Additional Information

<!-- ADDITIONAL INFORMATION START - Add custom information below this line -->

This graph break happens when you call `hasattr(obj, name)` on a value whose
type Dynamo cannot introspect for attributes. This commonly happens with lazy
builtin iterators (`map`, `zip`, `filter`), `slice`, `super()` objects, and
similar types. A frequent source is duck-typing helpers that branch on
`hasattr(...)` and get handed one of these objects.

Example code that causes the graph break is:
```python
import torch


def collate(values):
    # Duck-typing: branch on whether we can cheaply get a length.
    if hasattr(values, "__len__"):
        return torch.zeros(len(values))
    return torch.zeros(1)


@torch.compile(fullgraph=True)
def fn(x):
    # `map(...)` is a lazy iterator that Dynamo cannot introspect, so the
    # `hasattr` call inside `collate` breaks the graph.
    doubled = map(lambda v: v * 2, [1.0, 2.0, 3.0])
    return x + collate(doubled).sum()


fn(torch.randn(3))
```

The cleanest workaround is to materialize the lazy iterator into a type Dynamo
understands (e.g. a `list`) before calling `hasattr` on it. `hasattr` is
supported on `list`, `tuple`, `dict`, tensors, modules, and user-defined
objects:
```python
import torch


def collate(values):
    if hasattr(values, "__len__"):
        return torch.zeros(len(values))
    return torch.zeros(1)


@torch.compile(fullgraph=True)
def fn(x):
    # A list supports `hasattr`/`len`, so tracing proceeds without a break.
    doubled = list(map(lambda v: v * 2, [1.0, 2.0, 3.0]))
    return x + collate(doubled).sum()


fn(torch.randn(3))
```

Alternatively, if the `hasattr` check is only deciding control flow that is
fixed at trace time, hoist it out of the compiled region (e.g. compute it in
eager code and pass the result in), so Dynamo never has to introspect the
unsupported object.

<!-- ADDITIONAL INFORMATION END -->


[Click here to add Additional Info](https://github.com/meta-pytorch/compile-graph-break-site/edit/main/docs/gb/gb0150.md)

[Back to Registry](../index.html)
