...
For non-export cases, logging.Logger methods are only supported if the logger has a source and the method is registered as reorderable.
...
simplest fix is to move it outside the compiled region:
```python
import logging
import torch
